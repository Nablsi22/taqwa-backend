...
if (!url || !/^postgres/.test(url))
    throw new Error('ABORT: bad AUDIT_DATABASE_URL');
...
async function main() {
    ...
    for (const r of recs) {
        ...
        if (!(r.startSurah && r.startAya && r.endSurah && r.endAya)) {
            noData++;
            continue;
        }
        ...
        ayaSame++;
        const fresh = (0, quran_ayat_pages_util_1.calculatePages)(r.startSurah, r.startAya, r.endSurah, r.endAya);
        if (fresh === 0 && old > 0)
            zeroNew++;
        if (old > 0 && Math.abs(fresh - old) / old > 0.15) {
            suspicious.push(`${r.startSurah}:${r.startAya}-${r.endAya}  stored=${old}  recomputed=${fresh.toFixed(3)}`);
        }
    }
