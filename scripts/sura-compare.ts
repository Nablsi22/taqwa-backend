...
import { getSuraByNumber } from '../src/quran-metadata/quran-metadata';
...
for (const s of TOP) {
  ...
  const cur = getSuraByNumber(s);
  if (!meta || !cur) continue;
  const precise = calculatePages(s, 1, s, meta.numAyas);
  const old = cur.totalPages;
  const pct = ((precise - old) / old) * 100;
  const k = COUNT[s] ?? 0;
  wOld += old * k; wNew += precise * k; wN += k;
  console.log(
    (String(s) + ' ' + meta.nameAr).padEnd(18) + '| ' +
